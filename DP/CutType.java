...
public class CutType{
    ...
    class mcmPair{
        ...
        mcmPair(int len, String str){
            this.len = len;
            this.str = str;
        }

    }
    public static mcmPair mcmPrintAswer(int[] arr, int si, int ei, mcmPair[][] dp) {
        if (si + 1 == ei) {
            char ch = (char)((si) + (int)'A');

            return dp[si][ei] = new mcmPair(0, "" + ch);
        }

        if (dp[si][ei] != null)
            return dp[si][ei];

        mcmPair myPair = new mcmPair();

        for (int cut = si + 1; cut < ei; cut++) {
            mcmPair lans = mcmPrintAswer(arr, si, cut, dp);
            mcmPair rans = mcmPrintAswer(arr, cut, ei, dp);

            int num = lans.len + arr[si] * arr[cut] * arr[ei] + rans.len;
            if(myPair.len > num){
                myPair.len = num;
                myPair.str = "(" + lans.str + rans.str +")";
            }
            
        }

        return dp[si][ei] = myPair;
    }
